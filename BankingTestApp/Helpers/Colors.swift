//
//  Colors.swift
//  BankingTestApp
//
//  Created by Overlord on 28.05.2024.
//

import SwiftUI

extension Color {
    static let transactionIconBackground = Color(red: 0.965, green: 0.969, blue: 0.976) // #f6f7f9
    static let transactionAmountColorCommon = Color(red: 0.169, green: 0.173, blue: 0.224) // #2b2c39
    static let transactionAmountColorDeclined = Color(red: 0.494, green: 0.518, blue: 0.576) // #7e8493
    static let transactionSuccessColor = Color(red: 0, green: 0.675, blue: 0.31) // #00ac4f
    static let transactionDeclinedColor = Color(red: 0.925, green: 0.329, blue: 0.329) // #ec5454
}
